...
// edge and falling edge of  of PF0 (T0CCP0)
// Daniel Valvano
// May 5, 2015
...
void Sensor_Init(void){
  ...
  SYSCTL_RCGCGPIO_R |= 0x30;       // activate port E and port F
                                   // allow time to finish activating
	
	movement_Detected = 0;
  Starting = 0;                       // first will be wrong
	Done = 0;
  GPIO_PORTF_DIR_R &= ~0x01;       // make PF0 in
  GPIO_PORTF_AFSEL_R |= 0x01;      // enable alt funct on PF0/T0CCP0
...
void Timer0A_Handler(void){
  TIMER0_ICR_R = TIMER_ICR_CAECINT;// acknowledge timer0A capture match
	if(!Starting) {
		PW = 0;
		Starting = TIMER0_TAR_R;
	}
	else {
		PW = (TIMER0_TAR_R - Starting)&0x00FFFFFF; // from rise to fall
		Starting = 0;
		Done = 1;
	}
